...
kbn.getDecimalsForValue = function(value) {
  var delta = value / 2;
  ...
  result.scaledDecimals = result.decimals - Math.floor(Math.log(size) / Math.LN10) + 1;
...
kbn.formatValue = function(value, unit) {
  ...
  if(formatFunc){
    var decimalInfo = kbn.getDecimalsForValue(value);
