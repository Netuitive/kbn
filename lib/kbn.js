...
kbn.formatBuilders.simpleCountUnit = function(symbol) {
  ...
  return function(size, decimals, scaledDecimals) {
    ...
    var scaled = scaler(size, decimals, scaledDecimals);
    return {value:scaled, symbol:symbol};
  };
};
...
kbn.valueFormats.ns = function(size, decimals, scaledDecimals) {
  ...
  if (Math.abs(size) < 1000) {
    return {value: kbn.toFixed(size, decimals), symbol: 'ns'};
  } else if (Math.abs(size) < 1000000) {
...
kbn.findUnitFormat = function(unit) {
  ...
  formatFunc = kbn.valueFormats[unit]
  if (formatFunc) {
    return formatFunc;
  }
  // AWS units
  switch (unit) {
    case 'Bytes':
      formatFunc = kbn.valueFormats['bytes'];
      break;
    case 'Seconds':
      formatFunc = kbn.valueFormats['s']
      break;
    case 'Percent':
      formatFunc = kbn.valueFormats['percent']
      break;
  }
  if(formatFunc){
    return formatFunc;
  ...
  // try without case
  formatFunc = kbn.valueFormats[unit.toLowerCase()];
  if(formatFunc){
    return formatFunc;
  }
...
kbn.formatValue = function(value, unit) {
  ...
  if(formatFunc){
    ...
    if(result && result.value){
      ...
      // trailing 0's
      var val = new Number(new Number(result.value).toPrecision(4));
      return val + ' ' + result.symbol;
    }else{
      return value;
    }
  }
